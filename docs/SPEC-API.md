...
public class ShoppingItemDocument
{
    ...
    /// "high" | "medium" | "low". Null for items added manually.
    /// </summary>
...
Runtime configuration and feature flags live in a single Cosmos document in the `AppConfig` container (id `app-config`, partition key `/id`). The doc holds a flat `settings` dictionary keyed by `IConfiguration` paths (e.g. `PantryPunk:Claude:Model`, `FeatureManagement:RealtimeSync:EnabledFor:0:Name`).
...
```jsonc
"FeatureManagement:RealtimeSync:EnabledFor:0:Name": "Microsoft.Targeting",
"FeatureManagement:RealtimeSync:EnabledFor:0:Parameters:Audience:DefaultRolloutPercentage": "0",
"FeatureManagement:RealtimeSync:EnabledFor:0:Parameters:Audience:Groups:0:Name": "subscribers",
"FeatureManagement:RealtimeSync:EnabledFor:0:Parameters:Audience:Groups:0:RolloutPercentage": "0"
```
...
var isEnabled = await _featureManager.IsEnabledAsync("RealtimeSync", context);
```
...
if (!await _featureManager.IsEnabledAsync("AppAttest"))
...
- **Rate limiting** — apply rate limiting middleware (`Microsoft.AspNetCore.RateLimiting`) on AI endpoints (`/api/shopping-list/items/photo`) to protect Claude API costs. Suggested: 30 requests per user per minute.
- **Input validation** — use Data Annotations or FluentValidation on all request DTOs. Reject requests with oversized payloads.
- **Max upload size** — enforce a 3MB limit on photo uploads via Kestrel in `Program.cs`:
  ```csharp
...
> All `PantryPunk:*` and `FeatureManagement:*` keys (Claude model, rate limits, share code expiry, feature flags) live in the `AppConfig` Cosmos document — not in `appsettings.json`. The Claude API key and RevenueCat webhook secret live in Key Vault and are loaded at startup.
